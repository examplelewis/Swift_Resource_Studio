...
protocol GYDownloaderDelegate: AnyObject {
    func downloaderDidFinish(_ downloader: GYDownloader)
}

class GYDownloader: Equatable {
    weak var delegate: GYDownloaderDelegate?
    
    let setting: GYDownloadSetting
    private var URLs: [String] // 输入的链接, 不会变
    let txtFilePath: String? // 包含下载地址的 txt 文件路径
    ...
    // MARK: Equatable
    static func == (lhs: GYDownloader, rhs: GYDownloader) -> Bool {
        return lhs === rhs
    }
}
